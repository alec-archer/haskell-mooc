...
firstHalf str = take index str 
  where index
          | even (length str) = div (length str) 2
          | otherwise = (div (length str) 2) + 1
...
joinToLength n strs = filter (\s -> length s == n) [first ++ second | first <- strs, second <- strs]
...
-- multiCompose [] = id
-- multiCompose (x:xs) = x . (multiCompose xs)
...
-- multiApp f gs = f . multiApp' gs
--   where multiApp' [] x = []
--         multiApp' (g:gs) x = g x : (multiApp' gs x)
-- multiApp f gs x = f . [g x | g <- gs]
-- multiApp f gs x = f [g x | g <- gs]
-- multiApp f (g:gs) x = [f i | i <- (multiApp' gs (g x))]
--   where multiApp' [] x = [x]
--         multiApp' (g:gs) x = x : (multiApp' gs (g x))
...
interpreter commands = interpreter' commands 0 0
  where interpreter' [] _ _  = [] 
        interpreter' (c:cs) x y = case c of "up" -> interpreter' cs x (y + 1)
