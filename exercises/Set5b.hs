...
treeMax Empty = 0
treeMax (Node a t1 t2) = max (max a (treeMax t1)) (treeMax t2)
...
walk [] (Node a t1 t2) = Just a
walk (s:steps) (Node a t1 t2) = case s of
                                  StepL -> walk steps t1
                                  StepR -> walk steps t2
...
set _ _ Empty = Empty
set [] x (Node _ t1 t2) = (Node x t1 t2)
set (p:path) val (Node a t1 t2) = case p of
                                    StepL -> (Node a (set path val t1) t2)
                                    StepR -> (Node a t1 (set path val t2))
...
-- search val t = search' val t []
-- 
-- search' :: Eq a => a -> Tree a -> [Step] -> Maybe [Step]
-- search' _ Empty _ = Nothing
-- search' val (Node a t1 t2) steps
--   | val == a = Just steps
--   | otherwise = case (search' val t1 (steps ++ [StepL])) of
--                   Nothing -> case (search' val t2 (steps ++ [StepR])) of
--                                Nothing -> Nothing
--                                x -> x 
--                   x -> x
