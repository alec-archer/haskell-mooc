...
instance Eq Name where
  Name s1 == Name s2
    | map Data.Char.toLower s1 == map Data.Char.toLower s2 = True
    | otherwise = False
...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  -- LNode _ a1 == Empty = False
  -- Empty == LNode _ a2 = False
  -- LNode val1 a1 == LNode val2 a2
  --   | val1 /= val2 = False
  --   | otherwise = a1 == a2
...
instance Price a => Price [a] where
  price items = foldr (+) 0 (map price items)
...
simplify (RationalNumber a b) = RationalNumber (div a x) (div b x)
  where x = gcd a b
...
instance Num RationalNumber where
  RationalNumber a b + RationalNumber c d = simplify (RationalNumber (d*a + b*c) (d*b))
  RationalNumber a b * RationalNumber c d = simplify (RationalNumber (a * c) (b * d))
  abs (RationalNumber a b)
    | a < 0 = RationalNumber (-1 * a) b
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 t = t
  stepMany count t = stepMany (count - 1) (step t)
