...
data State = Start | Error | Finished | Eggs | EggsFlour | EggsSugar | AllIngredients | Mixed 
  deriving (Eq,Show)
...
passwordAllowed pw (MinimumLength minLength) = length pw >= minLength
passwordAllowed pw (ContainsSome string) = any (\c -> elem c pw) string
passwordAllowed pw (DoesNotContain string) = not $ passwordAllowed pw (ContainsSome string)
-- passwordAllowed pw (ContainsSome string) = elem True (map (\x -> elem x pw) string)
-- passwordAllowed pw (DoesNotContain string) = notElem True (map (\x -> elem x pw) string)
