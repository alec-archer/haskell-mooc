...
rgb col = case col of
            ...
            Green -> [0,1,0]
            Blue -> [0,0,1]
            Mix col1 col2 -> map (/2) (zipWith (+) (rgb col1) (rgb col2))
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne rest) = 1 + fromNat rest
...
fromBin b = fromBin' 0 b
  where fromBin' _ End = 0 
        fromBin' count (O rest) = fromBin' (count + 1) rest
        fromBin' count (I rest) = 2^count + fromBin' (count + 1) rest
