...
prettyPrint size queens = helper (1,1) ""
  where helper (row, col) str
          | col > size = helper (nextRow (row,col)) (str ++ "\n")
          | row > size = str
          | elem (row,col) queens = helper (nextCol (row,col)) (str ++ "Q")
          | otherwise = helper (nextCol (row,col)) (str ++ ".")
...
sameAntidiag (i,j) (k,l) = (abs (i - k) == abs (j - l) && signum (i - k) /= signum (j - l)) || (i == k && j == l)
...
danger _ [] = False
danger candidate (q:queens)
  | sameRow candidate q || sameCol candidate q || sameDiag candidate q || sameAntidiag candidate q = True 
  | otherwise = danger candidate queens
...
prettyPrint2 size queens = helper (1,1) "" 
  where helper (row, col) str
          | col > size = helper (nextRow (row,col)) (str ++ "\n")
          | row > size = str
          | elem (row,col) queens = helper (nextCol (row,col)) (str ++ "Q")
          | danger (row,col) queens = helper (nextCol (row,col)) (str ++ "#")
          | otherwise = helper (nextCol (row,col)) (str ++ ".")
...
fixFirst _ [] = Nothing
fixFirst n ((i,j):queens)
  | j > n = Nothing
  | danger (i,j) queens = fixFirst n ((nextCol (i,j)):queens)
  | otherwise = Just ((i,j):queens)
...
continue [] = []
continue queens = (nextRow (head queens)):queens

backtrack :: Stack -> Stack
backtrack [] = []
backtrack [q] = []
backtrack (_:q:queens) = (nextCol q):queens
...
step _ [] = []
step n stack
  | isComplete stack = tail stack 
  | otherwise = case (fixFirst n stack) of
                  (Just s) -> step n (continue s)
                  Nothing -> step n (backtrack stack)
    where isComplete ((i,_):stack) = i > n
